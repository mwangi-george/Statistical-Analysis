...
```{r eval=FALSE}
# create a report using the data in vgsales_df, output the report in html format, name the report vgsales_eda.html, and save the report in the current working directory
create_report(
  data = vgsales_df, 
...
vgsales_df_clean <- vgsales_df %>%
  ...
  # also deselect status because it contains only one value input(1) confirm with `count(vgsales_df, status)`
...
# we can now proceed to specify a model to fit the data with the parsnip package
```
